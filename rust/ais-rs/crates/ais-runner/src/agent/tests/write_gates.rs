...
fn write_gate_validation_requires_token_decimals_when_asset_input_lacks_decimals() {
    ...
    let error =
        super::validate_segment_write_gates(&segment, &candidate_context, Some(&input_store))
...
#[test]
fn write_gate_validation_accepts_store_backfill_to_inputs_token_decimals() {
    let segment: ais_sdk::documents::PlanSketchSegment = serde_json::from_value(json!({
        "segment_id": "seg-transfer",
        "cursor_in": "0",
        "cursor_out": "1",
    ...
    let input_store = InputStore::default();
    super::validate_segment_write_gates(
        &segment,
        &candidate_context,
        Some(&input_store),
    )
    .expect("query store to inputs.token.decimals should satisfy decimals check");
}
