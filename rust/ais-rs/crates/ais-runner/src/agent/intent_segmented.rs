    ...
    fn default() -> Self {
        Self {
            ...
            base_rules: vec![
                ...
                // -- Domain --
                "assert/branch/until/retry are PlanSketch control-step semantics, not catalog candidates. candidate_ref is required for query/action steps, optional for assert/branch. step.chain is optional when the segment has one deterministic chain, but required for query/action steps when planning across multiple chains.",
            ...
            phase_rules_grounding: vec![
                ...
                "Treat explicit token symbols/names as intent facts, not pack-policy bypasses. If later host diagnostics surface token_policy_signal, repair the token selection to satisfy pack policy instead of guessing an alternate symbol/address.",
            ...
            phase_rules_propose: vec![
                ...
                "Treat get_candidate_detail.semantic_hints as host-owned truth when present; those hints are only emitted when runner has a deterministic concrete chain for deployment-sensitive semantics. Do not reconstruct missing deployment/composite/pack semantics from catalog prose or invent fallback lowering.",
    ...
    let mut payload = json!({
        ...
        "grounding_contract": {
            ...
            "rules": [
                ...
                "Explicit token symbols/names can be grounded as intent facts, but pack token policy stays host-enforced. Do not fabricate alternate symbols/addresses to bypass later token_policy_signal diagnostics.",
