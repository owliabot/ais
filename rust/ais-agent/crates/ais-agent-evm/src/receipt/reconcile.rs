//! EVM receipt reconciliation entry points.
...
pub struct EvmReceiptReconciler;

impl ReceiptCapability for EvmReceiptReconciler {
    fn capability(&self) -> ChainCapability {
        ChainCapability {
            family: ChainFamily::Evm,
            kind: CapabilityKind::Receipt,
            implementation: "evm.receipt_reconciler",
        }
    }
    ...
    fn receipt(&self, query: &ReceiptQuery) -> Result<ReceiptView, ChainCapabilityError> {
        ...
        Ok(ReceiptView {
            ...
            payload: json!({"implementation": "evm.receipt_reconciler"}),
        })
    }
}
