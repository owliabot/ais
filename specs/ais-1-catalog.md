...
- 可省 token：默认输出 **index cards**；需要时再按 `ref` 拉取 **detail cards**（而不是把全量 protocol spec 塞进 prompt）。
...
### 2.1 Index-only contract (normative)

`ais-catalog/0.0.1` 是 **index-only** 权威文档：

- cards MUST be index cards.
- `level` 若出现 MUST 为 `"index"`。
- detail-only 字段（如 `params/returns/requires_queries`）不属于 catalog 主文档契约。

Detail retrieval is an explicit on-demand step:

- host/SDK SHOULD expose a separate detail lookup API by `ref`.
- detail payload is outside `ais-catalog/0.0.1` authority schema.

## 3. ActionCard

index card 推荐字段集合（最小可用）：

- `ref`: `protocol@version/actionId`
- `protocol`, `version`, `id`
- `description?`
- `risk_level`（required）, `risk_tags?`
...
- `providers?`: quote/routing provider allowlist 摘要
...
- JSON Schema: `schemas/0.0.2/catalog.schema.json`
