...
AIS-3 defines the on-chain registry contract for publishing, verifying, and discovering AIS specs. The registry ensures agents consume tamper-proof, verified protocol definitions.
...
/// Update an existing skill (owner only).
...
function addVerifier(address verifier) external;      // Owner only (Phase A/B)
...
```
Admin = single EOA (zhixian.eth)
Verifiers = [zhixian.eth]
...
Wallet Engine startup:
  1. Read AISRegistry.listVerified()
  2. For each verified skill:
     ...
     d. If mismatch → skip, log warning
...
Agent request flow:
  ...
  POST /execute { skill: "uniswap-v3", action: "swap", params: {...} }
    ...
    → Builds transaction per execution spec
...
4. **Frontrunning** — Registration is permissionless; someone could register "uniswap-v3" before the real team. Mitigation: verification process confirms real ownership.
...
| verify() | ~30,000 |
| getSkill() | View (free) |
| listVerified() | View (free, but scales with count) |
